export const lang = {
  // Common
  common: {
    mainNavBar: {
      menu: {
        faq: {
          en: "FAQ",
          es: "FAQ",
        },
        login: {
          en: "Log in",
          es: "Iniciar sesión",
        },
        profile: {
          en: "Profile",
          es: "Perfil",
        },
        tablesPlanner: {
          en: "Tables Planner",
          es: "Planificador",
        },
        logOut: {
          en: "Log out",
          es: "Salir",
        },
      },
    },
    mainFooter: {
      termsAndConditions: {
        en: "Terms and Conditions",
        es: "Términos y Condiciones",
      },
      privacyPolicy: {
        en: "Privacy Policy",
        es: "Política de Privacidad",
      },
      love: {
        en: "Made with",
        es: "Hecho con",
      },
    },
    failMessage: {
      en: "Something failed! Please try again.",
      es: "¡Algo falló! Inténtalo de nuevo.",
    },
    none: {
      en: "None",
      es: "Ninguno",
    },
  },
  // Home
  home: {
    meta: {
      title: {
        en: "Welcome",
        es: "Organiza tu boda paso a paso - Online | Wed Wed Weddings 💍",
      },
      description: {
        en:
          "Application to manage weddings tasks like guests invitations, tables planning or menus.",
        es:
          "¡Organizar una boda nunca ha sido tan fácil! Con nuestro Planificador de mesas de boda online podrás organizar tu lista de invitados y mucho más.          ",
      },
    },
    main: {
      subtitle: {
        en: `Your special day starts <span class="heading--detail">here</span> and <span class="heading--detail">now</span>`,
        es: `Tu día especial empieza <span class="heading--detail">aquí</span> y <span class="heading--detail">ahora</span>`,
      },
      button: {
        en: "Start",
        es: "Comenzar",
      },
    },
    whyUs: {
      title: {
        en: "Manage your wedding with us",
        es: "Organiza tu boda con nosotros",
      },
      images: [
        {
          en: "manage wedding invitations",
          es: "gestionar invitaciones de boda",
        },
        {
          en: "wedding planner",
          es: "organizador de bodas",
        },
      ],
      row1: [
        {
          en: "Antes de nada… ¡Felicidades por vuestro compromiso!",
          es: "Antes de nada… ¡Felicidades por vuestro compromiso!",
        },
        {
          en:
            "Si habéis llegado hasta aquí, es porque estáis muy cerca de dar el ‘’sí, quiero’’.",
          es:
            "Si habéis llegado hasta aquí, es porque estáis muy cerca de dar el ‘’sí, quiero’’.",
        },
        {
          en:
            "Somos conscientes de que necesitáis organizar muchas cosas antes de la boda y nuestra razón de ser, precisamente es la de aliviar algunos de esos dolores de cabeza a través de Wed Wed Weddings",
          es:
            "Somos conscientes de que necesitáis organizar muchas cosas antes de la boda y nuestra razón de ser, precisamente es la de aliviar algunos de esos dolores de cabeza a través de Wed Wed Weddings.",
        },
        {
          en:
            "Esta web está diseñada para conseguir hacer más llevaderas ciertas preparaciones que son fundamentales para que vuestra celebración salga a pedir de boca.",
          es:
            "Esta web está diseñada para conseguir hacer más llevaderas ciertas preparaciones que son fundamentales para que vuestra celebración salga a pedir de boca.",
        },
        {
          en: "¿Quieres saber cómo vamos a ayudaros?",
          es: "¿Quieres saber cómo vamos a ayudaros?",
        },
      ],
      row2: [
        {
          en:
            "A continuación, vamos a enumerar qué factores debéis tener en cuenta justo antes de comenzar a preparar una boda en Wed Wed Weddings.",
          es:
            "A continuación, vamos a enumerar qué factores debéis tener en cuenta justo antes de comenzar a preparar una boda en Wed Wed Weddings.",
        },
        {
          en: "💰 Fijar vuestro presupuesto.",
          es: "💰 Fijar vuestro presupuesto.",
        },
        {
          en:
            "🧮 Crear una lista de proveedores para el catering y sus precios.",
          es:
            "🧮 Crear una lista de proveedores para el catering y sus precios.",
        },
        {
          en: "💃 Organizar vuestra lista de invitados.",
          es: "💃 Organizar vuestra lista de invitados.",
        },
        {
          en: "🎁 Tener ideas para vuestra lista de regalos de boda.",
          es: "🎁 Tener ideas para vuestra lista de regalos de boda.",
        },
        {
          en:
            "👩‍💻 Disponer de tiempo de sobra para organizarlo todo adecuadamente.",
          es:
            "👩‍💻 Disponer de tiempo de sobra para organizarlo todo adecuadamente.",
        },
      ],
      timeline: [
        {
          en: "Create an account",
          es: "Crea una cuenta",
        },
        {
          en: "Add your wedding information",
          es: "Añade la información de tu boda",
        },
        {
          en: "Add your guests list",
          es: "Genera la lista de invitados",
        },
        {
          en: "Add your presents list",
          es: "Agrega regalos a tu lista de boda",
        },
        {
          en: "Organize guests per table",
          es: "Organiza a los invitados en las mesas",
        },
        {
          en: "Plan the catering",
          es: "Planifica el catering",
        },
      ],
      free: {
        en: "FREE",
        es: "GRATIS",
      },
    },
    findUs: {
      title: {
        en: "Find & Contact us!",
        es: "¡Encuéntranos!",
      },
      subtitle: {
        en:
          "Get our latest updates first!git <br />💍<br /><a href='https://wedwedweddings.com/faq'>Want to know something?</a><br/>Shoot us a message through...",
        es:
          "¡Sé siempre de los primeros en conocer las últimas novedades!<br />💍<br /><a href='https://wedwedweddings.com/faq'>¿Necesitas algo?</a> Envíanos un mensaje a través de...",
      },
    },
  },
  // Getting Started
  gettingStarted: {
    meta: {
      description: {
        en:
          "Application to manage weddings tasks like guests invitations, tables planning or menus.",
        es:
          "Sitio web para organizar bodas. Gestiona listas de invitados, listas de regalos y planifica las mesas y el catering.",
      },
    },
    steps: [
      {
        title: {
          en: "Profile data",
          es: "Datos del perfil",
        },
        description: {
          en: "Sólo necesitamos tu email y una contraseña para comenzar",
          es: "Sólo necesitamos tu email y una contraseña para comenzar",
        },
      },
      {
        title: {
          en: "Wedding data",
          es: "Datos de la boda",
        },
        description: {
          en: "Contadnos un poco más sobre vosotros",
          es: "Contadnos un poco más sobre vosotros",
        },
      },
      {
        title: {
          en: "Guests ⭐",
          es: "Invitados ⭐",
        },
        description: {
          en: "¿Quiénes van a ir a la boda?",
          es: "¿Quiénes van a ir a la boda?",
        },
      },
      {
        title: {
          en: "Presents",
          es: "Regalos",
        },
        description: {
          en: "¿Tenéis pensada una lista de regalos?",
          es: "¿Tenéis pensada una lista de regalos?",
        },
      },
      {
        title: {
          en: "Ready?",
          es: "¿Vamos?",
        },
        description: {
          en: "Start planning",
          es: "Empezar a planificar",
        },
      },
    ],
    registerForm: {
      meta: {
        title: {
          en: "Register",
          es: "Regístrate",
        },
      },
      title: {
        en: "🗝️ Register",
        es: "🗝️ Registro",
      },
      placeholders: {
        email: {
          en: "Please input email",
          es: "Escribe tu email",
        },
        password: {
          en: "Please input password",
          es: "Escribe tu contraseña",
        },
        warning: {
          en: "Mind that password will be shared for both to login!",
          es:
            "¡Ten en cuenta que quizás compartas esta contraseña con tu pareja!",
        },
        confirmPassword: {
          en: "Two passwords that you enter is inconsistent!",
          es: "¡Las contraseñas no coinciden!",
        },
      },
      validators: {
        email: {
          en: "The input is not valid email!",
          es: "¡El email no es correcto!",
        },
        password: {
          en: "Password must have at least 8 characters!",
          es: "¡La contraseña debe tener al menos 8 caracteres!",
        },
        confirmPassword: {
          en: "Please confirm password",
          es: "Confirma tu contraseña",
        },
      },
      termsAndConditions: {
        en: "I agree Terms and Conditions",
        es: "Acepto los Términos y Condiciones",
      },
      registerButton: {
        en: "Register",
        es: "Registrarse",
      },
      haveAccount: {
        en: "Have account?",
        es: "¿Ya estás registrado?",
      },
      forgotPassword: {
        en: "Forgot password?",
        es: "¿Has olvidado la contraseña?",
      },
    },
    weddingForm: {
      meta: {
        title: {
          en: "About your wedding",
          es: "Sobre tu boda",
        },
      },
      title: {
        en: "📝 Wedding",
        es: "📝 Boda",
      },
      placeholders: {
        spouse1: {
          en: "❤️ Spouse 1 @ Team 1",
          es: "❤️ Cónyuge 1 @ Team 1",
        },
        firstName1: {
          en: "First firstname",
          es: "Nombre",
        },
        lastName1: {
          en: "First lastname",
          es: "Apellido",
        },
        emoji1: {
          en: "Select emoji",
          es: "Elige un emoticono",
        },
        spouse2: {
          en: "❤️ Spouse 2 @ Team 2",
          es: "❤️ Cónyuge 2 @ Team 2",
        },
        firstName2: {
          en: "Second firstname",
          es: "Nombre",
        },
        lastName2: {
          en: "Second lastname",
          es: "Apellido",
        },
        emoji2: {
          en: "Select emoji",
          es: "Elige un emoticono",
        },
      },
      validators: {
        firstName1: {
          en: "Please input the first firstname!",
          es: "¡Escribe el nombre del primer cónyuge!",
        },
        lastName1: {
          en: "Please input the first lastname!",
          es: "¡Escribe el apellido del primer cónyuge!",
        },
        emoji1: {
          en: "Please input the first emoji!",
          es: "¡Elige el primer emoticono!",
        },
        firstName2: {
          en: "Please input the second firstname!",
          es: "¡Escribe el nombre del segundo cónyuge!",
        },
        lastName2: {
          en: "Please input the second lastname!",
          es: "¡Escribe el apellido del segundo cónyuge!",
        },
        emoji2: {
          en: "Please input the second emoji!",
          es: "¡Elige el segundo emoticono!",
        },
      },
      continueButton: {
        en: "Continue",
        es: "Continuar",
      },
    },
    guestsForm: {
      meta: {
        title: {
          en: "Add guests",
          es: "Añade invitados",
        },
      },
      title: {
        en: "💃🕺 Guests",
        es: "💃🕺 Invitados",
      },
      placeholders: {
        fullName: {
          en: "Guest full name",
          es: "Nombre y apellidos del invitado",
        },
        relative: {
          en: "Invited by",
          es: "Viene invitado por",
        },
        menu: {
          en: "Guest menu",
          es: "Menú del invitado",
        },
        email: {
          en: "Guest email",
          es: "Email del invitado",
        },
        warning: {
          en:
            "⭐ We'll send an email to this guest to get assistance confirmation ⭐",
          es:
            "⭐ Enviaremos un email a este invitado para que confirme su asistencia ⭐",
        },
      },
      validators: {
        fullName: {
          en: "Please input guest full name!",
          es: "¡Escribe el nombre y los apellidos!",
        },
        relative: {
          en: "Please select guest relative!",
          es: "¡Elige de parte de quien viene!",
        },
        menu: {
          en: "Please select guest menu!",
          es: "¡Elige el menú para el invitado!",
        },
        email: {
          en: "The input is not valid email!",
          es: "¡El email no es correcto!",
        },
      },
      addButton: {
        en: "Add guest",
        es: "Añadir invitado",
      },
      addSuccess: {
        en: "New Guest added!",
        es: "¡Invitado añadido!",
      },
      updateButton: {
        en: "Update guest",
        es: "Actualizar invitado",
      },
      updateSuccess: {
        en: "Guest updated!",
        es: "¡Invitado actualizado!",
      },
      later: {
        en: "You can add and update guests later too.",
        es: "Puedes continuar después si lo prefieres.",
      },
      continueButton: {
        en: "Continue",
        es: "Continuar",
      },
    },
    guest: {
      deleteConfirm: {
        en: "Delete this guest?",
        es: "¿Borrar invitado?",
      },
      deleteConfirmYes: {
        en: "Yes",
        es: "Sí",
      },
      deleteSuccess: {
        en: "Guest deleted!",
        es: "Invitado borrado!",
      },
    },
    presentsForm: {
      meta: {
        title: {
          en: "Add presents",
          es: "Añade regalos",
        },
      },
      title: {
        en: "🎁 Presents",
        es: "🎁 Regalos",
      },
      placeholders: {
        title: {
          en: "Present name",
          es: "Nombre del regalo",
        },
        boughtByPlaceholder: {
          en: "Assign to",
          es: "Asignar a",
        },
      },
      validators: {
        title: {
          en: "Please input present!",
          es: "¡Escribe el nombre del regalo!",
        },
      },
      addButton: {
        en: "Add present",
        es: "Añadir regalo",
      },
      addSuccess: {
        en: "New present added!",
        es: "¡Regalo añadido!",
      },
      updateButton: {
        en: "Update present",
        es: "Actualizar regalo",
      },
      updateSuccess: {
        en: "Present updated!",
        es: "¡Regalo actualizado!",
      },
      later: {
        en: "You can add and update presents later too.",
        es: "Puedes continuar después si lo prefieres.",
      },
      continueButton: {
        en: "Continue",
        es: "Continuar",
      },
    },
    present: {
      deleteConfirm: {
        en: "Delete this present?",
        es: "¿Borrar regalo?",
      },
      deleteConfirmYes: {
        en: "Yes",
        es: "Sí",
      },
      deleteSuccess: {
        en: "Present deleted!",
        es: "¡Regalo borrado!",
      },
    },
  },
  // Login
  login: {
    meta: {
      title: {
        en: "Good to see you",
        es: "Nos alegramos de verte",
      },
      description: {
        en:
          "Application to manage weddings tasks like guests invitations, tables planning or menus.",
        es:
          "Sitio web para organizar bodas. Gestiona listas de invitados, listas de regalos y planifica las mesas y el catering.",
      },
    },
    title: {
      en: "Hi! 😊",
      es: "¡Hola! 😊",
    },
    button: {
      en: "Log in",
      es: "Entrar",
    },
    noAccount: {
      en: "Sign up",
      es: "Crear cuenta",
    },
  },
  // Forgot Password
  forgotPassword: {
    meta: {
      title: {
        en: "Recover your password",
        es: "Recupera tu contraseña",
      },
      description: {
        en:
          "Application to manage weddings tasks like guests invitations, tables planning or menus.",
        es:
          "Sitio web para organizar bodas. Gestiona listas de invitados, listas de regalos y planifica las mesas y el catering.",
      },
    },
    title: {
      en: "🤔 Forgot password?",
      es: "🤔 ¿Has olvidado la contraseña?",
    },
    button: {
      en: "Recover password",
      es: "Recuperar contraseña",
    },
    submitSuccess: {
      en: "We sent you an email! 📧",
      es: "¡Te hemos mandado un email! 📧",
    },
  },
  // Reset Password
  resetPassword: {
    meta: {
      title: {
        en: "Recover password",
        es: "Recuperar contraseña",
      },
      description: {
        en:
          "Application to manage weddings tasks like guests invitations, tables planning or menus.",
        es:
          "Sitio web para organizar bodas. Gestiona listas de invitados, listas de regalos y planifica las mesas y el catering.",
      },
    },
    title: {
      en: "🤐 Recover password",
      es: "🤐 Recuperar contraseña",
    },
    button: {
      en: "Change",
      es: "Cambiar",
    },
    submitSuccess: {
      en: "Password changed!",
      es: "¡Contraseña cambiada!",
    },
  },
  // FAQ
  faq: {
    meta: {
      title: {
        en: "FAQ",
        es: "FAQ - Cómo organizar y planificar una Boda | Wed Wed Weddings 💍",
      },
      description: {
        en:
          "Application to manage weddings tasks like guests invitations, tables planning or menus.",
        es:
          "¿Dudas sobre cómo organizar y planificar tu boda con Wed Wed Weddings 💍? Este es el apartado de FAQ donde puedes resolver todas tus preguntas.",
      },
    },
    description: {
      en: "Pending",
      es: "Pendiente",
    },
    simple: [
      // #1 - Preguntas Frecuentes
      {
        title: {
          en: "What is Wed Wed Weddings?",
          es: "¿Qué es Wed Wed Weddings?",
        },
        description: {
          en: "Wed Wed Weddings es un ‘’wedding planner’’ gratuito online en el que podréis planificar la organización de vuestra boda paso a paso con nosotros. En Wed Wed Weddings, podréis crear y organizar las invitaciones y mesas de los invitados de forma totalmente gratuita, además de la lista de regalos y planificar el presupuesto del catering.",
          es: "Wed Wed Weddings es un ‘’wedding planner’’ gratuito online en el que podréis planificar la organización de vuestra boda paso a paso con nosotros. En Wed Wed Weddings, podréis crear y organizar las invitaciones y mesas de los invitados de forma totalmente gratuita, además de la lista de regalos y planificar el presupuesto del catering.",
        },
      },
      // #1 - Preguntas Frecuentes
      {
        title: {
          en: "How can you organize your wedding in Wed Wed Weddings",
          es: "¿Cómo planificar tu boda en Wed Wed Weddings?",
        },
        description: {
          en: "Para organizar tu boda de manera sencilla con nosotros, los pasos a seguir son los siguientes: debéis registraros en la web utilizando un solo correo electrónico, que será con el que se gestionará la boda; a continuación, crear una lista de invitados que podréis modificar y gestionar cuantas veces queráis; después, podéis crear vuestra lista de regalos de boda y asignarlos a los invitados y, por último, organizar a los invitados en las mesas del convite tantas veces como queráis.",
          es: "Para organizar tu boda de manera sencilla con nosotros, los pasos a seguir son los siguientes: debéis registraros en la web utilizando un solo correo electrónico, que será con el que se gestionará la boda; a continuación, crear una lista de invitados que podréis modificar y gestionar cuantas veces queráis; después, podéis crear vuestra lista de regalos de boda y asignarlos a los invitados y, por último, organizar a los invitados en las mesas del convite tantas veces como queráis.",
        },
      },
      // #1 - Preguntas Frecuentes
      {
        title: {
          en: "Which devices can I use to manage Wed Wed Weddings?",
          es: "¿En qué dispositivos puedo usar Wed Wed Weddings?",
        },
        description: {
          en: "Podéis organizar vuestra boda a través de cualquier dispositivo, ya sea móvil, tablet o pc. En móviles, hay ciertas gestiones que no se pueden realizar (organizar las mesas de los invitados, debéis realizar la gestión en cualquier otro dispositivo distinto a móvil).",
          es: "Podéis organizar vuestra boda a través de cualquier dispositivo, ya sea móvil, tablet o pc. En móviles, hay ciertas gestiones que no se pueden realizar (organizar las mesas de los invitados, debéis realizar la gestión en cualquier otro dispositivo distinto a móvil).",
        },
      },
      // #2 - Funciones del usuario
      {
        title: {
          en: "As an user, which actions can I take?",
          es: "¿Qué funciones tengo como usuario?",
        },
        description: {
          en: "Con vuestro usuario, podéis modificar la lista de regalos y asignar regalos a vuestros invitados, organizar los asientos y mesas de los invitados en el planificador de mesas, y gestionar las invitaciones de la boda, según la información que proporcionen los invitados, como nuevos invitados y sus menús. Podéis agregar manualmente más invitados dentro del Planificador de mesas.",
          es: "Con vuestro usuario, podéis modificar la lista de regalos y asignar regalos a vuestros invitados, organizar los asientos y mesas de los invitados en el planificador de mesas, y gestionar las invitaciones de la boda, según la información que proporcionen los invitados, como nuevos invitados y sus menús. Podéis agregar manualmente más invitados dentro del Planificador de mesas.",
        },
      },
      // #3 - Funciones del usuario
      {
        title: {
          en: "Do my partner and I use the same username to manage the wedding data?",
          es: "¿Usamos el mismo usuario mi pareja y yo para gestionar la boda?",
        },
        description: {
          en: "Tal y como indicamos en el registro de usuario de Wed Wed Weddings, la creación de la cuenta se realiza con un solo correo electrónico y el mismo usuario, será usado por la pareja para gestionar la boda.",
          es: "Tal y como indicamos en el registro de usuario de Wed Wed Weddings, la creación de la cuenta se realiza con un solo correo electrónico y el mismo usuario, será usado por la pareja para gestionar la boda.",
        },
      },
      // #4 - Funciones del usuario
      {
        title: {
          en: "How can I recover my password?",
          es: "¿Cómo recupero/modifico mi contraseña?",
        },
        description: {
          en: "En el menú que aparece en la pantalla de Inicio, debes pulsar el botón de ‘’Entrar’’ y aparecerá, debajo del botón ‘’Entrar’’, la opción de ‘’¿Has olvidado la contraseña?’’.  Debes introducir el correo con el que habéis creado la cuenta de Wed Wed Weddings y abrir el link que aparecerá en el correo que se te enviará. ¡Date prisa que este link solo es válido durante 10 minutos!",
          es: "En el menú que aparece en la pantalla de Inicio, debes pulsar el botón de ‘’Entrar’’ y aparecerá, debajo del botón ‘’Entrar’’, la opción de ‘’¿Has olvidado la contraseña?’’.  Debes introducir el correo con el que habéis creado la cuenta de Wed Wed Weddings y abrir el link que aparecerá en el correo que se te enviará. ¡Date prisa que este link solo es válido durante 10 minutos!",
        },
      },
      // #5 - Funciones del usuario
      {
        title: {
          en: "Can my partner see the modifications I make?",
          es: "¿Mi pareja puede ver las modificaciones que hago?",
        },
        description: {
          en: "Sí, aparecerán los datos modificados, pero no se notifica de ninguna manera los cambios que realices.",
          es: "Sí, aparecerán los datos modificados, pero no se notifica de ninguna manera los cambios que realices.",
        },
      },
      // #6 - Funciones del usuario
      {
        title: {
          en: "Where is located the wedding data?",
          es: "¿Dónde localizo todos los datos de mi boda?",
        },
        description: {
          en: "Todos los datos que habéis introducido o hayáis modificado para planear vuestra boda, ya sea la distribución de los invitados en sus respectivas mesas, la lista de los propios invitados o de los regalos y el presupuesto de los menús, aparecerán en la pestaña del Planificador de mesas.",
          es: "Todos los datos que habéis introducido o hayáis modificado para planear vuestra boda, ya sea la distribución de los invitados en sus respectivas mesas, la lista de los propios invitados o de los regalos y el presupuesto de los menús, aparecerán en la pestaña del Planificador de mesas.",
        },
      },
      // #7 - Planificador de mesas
      {
        title: {
          en: "How can the wedding guest tables be arranged?",
          es: "¿Cómo se pueden organizar las mesas de invitados de la boda?",
        },
        description: {
          en: "Podéis organizar en el planificador de mesas a los invitados de la boda como queráis. En las mesas, podéis incluir hasta 12 comensales y no hay límite para añadir número de mesas. No olvidéis incluir una mesa para niños, si tenéis pensado añadir una. Os aconsejamos organizar a los invitados de tu boda en alas, es decir, los invitados correspondientes a uno de los cónyuges en el lado en el que esa persona se sienta y hacer lo mismo con los del otro/a esposo/a.",
          es: "Podéis organizar en el planificador de mesas a los invitados de la boda como queráis. En las mesas, podéis incluir hasta 12 comensales y no hay límite para añadir número de mesas. No olvidéis incluir una mesa para niños, si tenéis pensado añadir una. Os aconsejamos organizar a los invitados de tu boda en alas, es decir, los invitados correspondientes a uno de los cónyuges en el lado en el que esa persona se sienta y hacer lo mismo con los del otro/a esposo/a.",
        },
      },
      // #8 - Planificador de mesas
      {
        title: {
          en: "Can I move a guest to another table?",
          es: "¿Puedo mover un invitado de una mesa a otra?",
        },
        description: {
          en: "Sí, dentro del planificador de mesas. Primero debéis eliminarlo del asiento que tenga asignado y seleccionar el nuevo lugar en el queráis que esa persona esté.",
          es: "Sí, dentro del planificador de mesas. Primero debéis eliminarlo del asiento que tenga asignado y seleccionar el nuevo lugar en el queráis que esa persona esté.",
        },
      },
      // #9 - Planificador de mesas
      {
        title: {
          en: "Are tables movable?",
          es: "¿Se pueden mover las mesas?",
        },
        description: {
          en: "Por ahora no se puede realizar esta acción, en las próximas implementaciones haremos esto posible.",
          es: "Por ahora no se puede realizar esta acción, en las próximas implementaciones haremos esto posible.",
        },
      },
      // #10 - Planificador de mesas
      {
        title: {
          en: "Can I change table's name or number?",
          es: "¿Puedo cambiar el nombre o número de la mesa?",
        },
        description: {
          en: "Por ahora no se puede realizar esta acción, en las próximas implementaciones haremos esto posible.",
          es: "Por ahora no se puede realizar esta acción, en las próximas implementaciones haremos esto posible.",
        },
      },
      // #11 - Planificador de mesas
      {
        title: {
          en: "Can I move multiple guests from a table to another?",
          es: "¿Puedo mover varios invitados de una mesa a otra?",
        },
        description: {
          en: "Por ahora no se puede realizar esta acción, en las próximas implementaciones haremos esto posible",
          es: "Por ahora no se puede realizar esta acción, en las próximas implementaciones haremos esto posible.",
        },
      },
      // #12 - Lista de invitados
      {
        title: {
          en: "How can I include a new guest?",
          es: "¿Cómo creo un invitado?",
        },
        description: {
          en: "Tienes dos opciones: Al crear la cuenta de usuario de Wed Wed Weddings, podéis añadir todos los invitados que quieras. NO se pueden modificar los datos de los invitados en este paso, pero sí podéis eliminarlos e introducir sus datos de nuevo. Debéis llegar al ‘’Planificador de mesas’’ para poder modificarlos y no eliminar datos. Una vez creada la cuenta, dentro del planificador de mesas, en la lista de nombres. Primero, debéis seleccionar el nombre de la persona y clicar sobre él. Después, modificar los datos que queráis y pulsar en el botón de ‘’Actualizar invitado’’",
          es: "Tienes dos opciones: Al crear la cuenta de usuario de Wed Wed Weddings, podéis añadir todos los invitados que quieras. NO se pueden modificar los datos de los invitados en este paso, pero sí podéis eliminarlos e introducir sus datos de nuevo. Debéis llegar al ‘’Planificador de mesas’’ para poder modificarlos y no eliminar datos. Una vez creada la cuenta, dentro del planificador de mesas, en la lista de nombres. Primero, debéis seleccionar el nombre de la persona y clicar sobre él. Después, modificar los datos que queráis y pulsar en el botón de ‘’Actualizar invitado’’.",
        },
      },
      // #13 - Lista de invitados
      {
        title: {
          en: "How can I modify the data of a guest?",
          es: "¿Cómo modifico los datos de un invitado?",
        },
        description: {
          en: "Dentro del planificador de mesas, en la lista de nombres. Primero, debéis seleccionar el nombre de la persona y clicar sobre él. Después, modificad los datos que queráis y pulsad en el botón de ‘’Actualizar invitado''.",
          es: "Dentro del planificador de mesas, en la lista de nombres. Primero, debéis seleccionar el nombre de la persona y clicar sobre él. Después, modificad los datos que queráis y pulsad en el botón de ‘’Actualizar invitado''.",
        },
      },
      // #14 - Lista de invitados
      {
        title: {
          en: "How can I send my wedding invitations from Wed Wed Weddings?",
          es: "¿Cómo envío desde Wed Wed Weddings las invitaciones de boda?",
        },
        description: {
          en: "Si tenéis el correo electrónico de los invitados, se pueden enviar las invitaciones al principio, a la hora de crear la lista de invitados una vez estáis empezando a registrar la boda. Si no tenéis el correo electrónico de algún invitado, siempre podéis invitarlo más tarde dentro del planificador de mesas o al añadir a un nuevo invitado.",
          es: "Si tenéis el correo electrónico de los invitados, se pueden enviar las invitaciones al principio, a la hora de crear la lista de invitados una vez estáis empezando a registrar la boda. Si no tenéis el correo electrónico de algún invitado, siempre podéis invitarlo más tarde dentro del planificador de mesas o al añadir a un nuevo invitado. ",
        },
      },
      // #15 - Lista de invitados
      {
        title: {
          en: "Can I sort guests by categories?",
          es: "¿Se puede filtrar a los invitados por categorías?",
        },
        description: {
          en: "Sí, se pueden filtrar. En el panel donde se encuentra la Lista de invitados de la boda, aparecen todos los filtros que se pueden seleccionar. Se pueden filtrar para ver si, por ejemplo, los invitados que pertenecen a la categoría ‘’Gimnasio’’ que vienen de parte de la novia, están sentados todos juntos en una mesa y también, por si falta alguien por sentar de esa categoría en concreto.",
          es: "Sí, se pueden filtrar. En el panel donde se encuentra la Lista de invitados de la boda, aparecen todos los filtros que se pueden seleccionar. Se pueden filtrar para ver si, por ejemplo, los invitados que pertenecen a la categoría ‘’Gimnasio’’ que vienen de parte de la novia, están sentados todos juntos en una mesa y también, por si falta alguien por sentar de esa categoría en concreto.",
        },
      },
      // #16 - Lista de regalos
      {
        title: {
          en: "Can I modify gift list?",
          es: "¿Puedo modificar la lista de regalos?",
        },
        description: {
          en: "Tenéis dos opciones: Al crear la cuenta de usuario de Wed Wed Weddings, podéis añadir todos los regalos que queráis. NO se pueden modificar los datos de los regalos en este paso, debéis llegar al ‘’Planificador de mesas’’ para poder modificarlos. Una vez creada la cuenta, dentro del planificador de mesas, en la lista de los regalos. Primero, debéis seleccionar el regalo y clicar sobre él. Después, modificad los datos que quieras y, luego pulsar en el botón de ‘’Actualizar regalo’’.",
          es: "Tenéis dos opciones: Al crear la cuenta de usuario de Wed Wed Weddings, podéis añadir todos los regalos que queráis. NO se pueden modificar los datos de los regalos en este paso, debéis llegar al ‘’Planificador de mesas’’ para poder modificarlos. Una vez creada la cuenta, dentro del planificador de mesas, en la lista de los regalos. Primero, debéis seleccionar el regalo y clicar sobre él. Después, modificad los datos que quieras y, luego pulsar en el botón de ‘’Actualizar regalo’’.",
        },
      },
      // #17 - Lista de regalos
      {
        title: {
          en: "Can guests assign presents to theirselves?",
          es: "¿Pueden los invitados asignarse regalos?",
        },
        description: {
          en: "Debéis asignarlos vosotros mismos. En las próximas implementaciones se podrán asignar los propios invitados los regalos de la lista de bodas desde la web de Wed Wed Weddings.",
          es: "Debéis asignarlos vosotros mismos. En las próximas implementaciones se podrán asignar los propios invitados los regalos de la lista de bodas desde la web de Wed Wed Weddings.",
        },
      },
     
      // #18 - Presupuesto del catering
      {
        title: {
          en: "What is it the menu budget calculator for?",
          es: "¿Para qué sirve el presupuesto del catering?",
        },
        description: {
          en: "A muchas parejas les resulta muy útil para ir controlando el presupuesto final que les va a suponer el servicio de catering y para hacer una comparativa de precios de distintos caterings. También resulta muy útil para realizar una estimación de cuántos menús de carácter especial, se requieren para pedir al catering y poder comprobar también si se producen variaciones de precios.",
          es: "A muchas parejas les resulta muy útil para ir controlando el presupuesto final que les va a suponer el servicio de catering y para hacer una comparativa de precios de distintos caterings. También resulta muy útil para realizar una estimación de cuántos menús de carácter especial, se requieren para pedir al catering y poder comprobar también si se producen variaciones de precios.",
        },
      },
      // #19 - Presupuesto del catering
      {
        title: {
          en: "How can I use the menu budget calculator?",
          es: "¿Cómo uso la calculadora del presupuesto del catering?",
        },
        description: {
          en: "Debéis introducir vosotros mismos los precios y el número de menús de cada uno de ellos. Esta herramienta, según los datos que vayáis metiendo, calculará automáticamente el presupuesto de vuestro catering.",
          es: "Debéis introducir vosotros mismos los precios y el número de menús de cada uno de ellos. Esta herramienta, según los datos que vayáis metiendo, calculará automáticamente el presupuesto de vuestro catering.",
        },
      },
      // ⚠️ This has to be the last!
      {
        title: {
          en: "Do you still have doubts?",
          es: "¿Aún con dudas?",
        },
        description: {
          en: "You can contact with us through social media!",
          es: "Puedes contactar con nosotros a través de las redes sociales.",
        },
      },
    ],
    complex: {},
  },
  // Not Found
  notFound: {
    meta: {
      title: {
        en: "Ooops!",
        es: "¡Uuups!",
      },
      description: {
        en: "Bla bla bla",
        es: "Bla bla bla",
      },
    },
  },
  // Tables Planner
  tablesPlanner: {
    meta: {
      title: {
        en: "Tables Planner",
        es: "Planificador",
      },
    },
    menu: {
      budget: {
        en: "Catering Budget",
        es: "Presupuesto del catering",
      },
      guests: {
        en: "Guests",
        es: "Invitados",
      },
      presents: {
        en: "Presents",
        es: "Regalos",
      },
      addButton: {
        en: "Add",
        es: "Añadir",
      },
    },
    planner: {
      title: {
        en: "Tables Planner",
        es: "Planificador",
      },
      addTableButton: {
        en: "Add table",
        es: "Añadir mesa",
      },
      table: {
        en: "Table",
        es: "Mesa",
      },
      deleteConfirm: {
        en: "Delete this table?",
        es: "¿Borrar mesa?",
      },
      deleteConfirmYes: {
        en: "Yes",
        es: "Sí",
      },
      deleteSuccess: {
        en: "Table deleted!",
        es: "¡Mesa borrada!",
      },
      seatSelectGuest: {
        en: "Select guest",
        es: "Elige un invitado",
      },
    },
  },
  // Privacy Policy
  privacyPolicy: {
    meta: {
      title: {
        en: "Privacy Policy",
        es: "Política de Privacidad",
      },
      description: {
        en:
          "Application to manage weddings tasks like guests invitations, tables planning or menus.",
        es:
          "Sitio web para organizar bodas. Gestiona listas de invitados, listas de regalos y planifica las mesas y el catering.",
      },
    },
    title: {
      en: "Privacy Policy",
      es: "Política de Privacidad",
    },
    description: {
      en: "",
      es: `Wed Wed Weddings es el RESPONSABLE del tratamiento de los datos personales del USUARIO y le informa de que estos datos serán tratados de conformidad con lo dispuesto en el Reglamento (UE) 2016/679, de 27 de abril (GDPR), y la Ley Orgánica 3/2018, de 5 de diciembre (LOPDGDD).`,
    },
    simple: [
      // #1
      {
        title: {
          en: "",
          es: "¿Para qué tratamos tus datos personales?",
        },
        description: {
          en: "",
          es: `Para mantener una relación comercial con el usuario. Las operaciones previstas para realizar el tratamiento son: 1.- Remisión de comunicaciones comerciales publicitarias por e-mail, redes sociales o cualquier otro medio electrónico o físico, presente o futuro, que posibilite realizar comunicaciones comerciales. Estas comunicaciones serán realizadas por el RESPONSABLE y estarán relacionadas con sus productos y servicios, o de sus colaboradores o proveedores, con los que este haya alcanzado algún acuerdo de promoción. En este caso, los terceros nunca tendrán acceso a los datos personales. 2.- Realizar estudios de mercado y análisis estadísticos. 3.- Tramitar encargos, solicitudes, dar respuesta a las consultas o cualquier tipo de petición que sea realizada por el usuario a través de cualquiera de las formas de contacto que se ponen a su disposición en la página web de el RESPONSABLE.`,
        },
      },
      // #2
      {
        title: {
          en: "",
          es: "¿Por qué motivo podemos tratar tus datos personales?",
        },
        description: {
          en: "",
          es: `Porque el tratamiento está legitimado por el artículo 6 del GDPR de la siguiente forma: 1.- Con el consentimiento del usuario: remisión de comunicaciones comerciales y del boletín informativo. 2.- Por interés legítimo de el RESPONSABLE: realizar estudios de mercado, análisis estadísticos, etc. y tramitar encargos, solicitudes, etc. a petición del usuario.`,
        },
      },
      // #3
      {
        title: {
          en: "",
          es: "¿Durante cuánto tiempo guardaremos tus datos personales?",
        },
        description: {
          en: "",
          es: `Se conservarán durante no más tiempo del necesario para mantener el fin del tratamiento o existan prescripciones legales que dictaminen su custodia y cuando ya no sea necesario para ello, se suprimirán con medidas de seguridad adecuadas para garantizar la anonimización de los datos o la destrucción total de los mismos.`,
        },
      },
      // #4
      {
        title: {
          en: "",
          es: "¿Cuáles son tus derechos?",
        },
        description: {
          en: "",
          es: `Los derechos que asisten al usuario son: 1.- Derecho a retirar el consentimiento en cualquier momento. 2.- Derecho de acceso, rectificación, portabilidad y supresión de sus datos, y de limitación u oposición a su tratamiento.`,
        },
      },
    ],
  },
  // Terms and Conditions
  termsAndConditions: {
    meta: {
      title: {
        en: "Terms and Conditions",
        es: "Términos y Condiciones",
      },
      description: {
        en:
          "Application to manage weddings tasks like guests invitations, tables planning or menus.",
        es:
          "Sitio web para organizar bodas. Gestiona listas de invitados, listas de regalos y planifica las mesas y el catering.",
      },
    },
    title: {
      en: "Terms and Conditions",
      es: "Términos y Condiciones",
    },
    description: {
      en: "",
      es: `El presente aviso legal regula el uso y utilización del sitio wedwedweddings.com, del que es titular Wed Wed Weddings (en adelante, EL PROPIETARIO DE LA WEB).
      La navegación por el sitio web de EL PROPIETARIO DE LA WEB le atribuye la condición de USUARIO del mismo y conlleva su aceptación plena y sin reservas de todas y cada una de las condiciones publicadas en este aviso legal, advirtiendo de que dichas condiciones podrán ser modificadas sin notificación previa por parte de EL PROPIETARIO DE LA WEB, en cuyo caso se procederá a su publicación y aviso con la máxima antelación posible.
      Por ello es recomendable leer atentamente su contenido en caso de desear acceder y hacer uso de la información y de los servicios ofrecidos desde este sitio web.
      El usuario, además, se obliga a hacer un uso correcto del sitio web de conformidad con las leyes, la buena fe, el orden público, los usos del tráfico y el presente Aviso Legal, y responderá frente a EL PROPIETARIO DE LA WEB o frente a terceros, de cualesquiera daños y perjuicios que pudieran causarse como consecuencia del incumplimiento de dicha obligación.
      Cualquier utilización distinta a la autorizada está expresamente prohibida, pudiendo EL PROPIETARIO DE LA WEB denegar o retirar el acceso y su uso en cualquier momento.
      `,
    },
    list: [
      // #1
      {
        en: "",
        es: `1.- Los derechos de propiedad intelectual de la página wedwedweddings.com, su código fuente, diseño, estructura de navegación, bases de datos y los distintos elementos en él contenidos son titularidad de EL PROPIETARIO, a quien corresponde el ejercicio exclusivo de los derechos de explotación de los mismos en cualquier forma y, en especial, los derechos de reproducción, distribución, comunicación pública y transformación.`,
      },
      // #2
      {
        en: "",
        es: `2.- Estas condiciones generales regulan el acceso y utilización del sitio web wedwedweddings.com que EL PROPIETARIO pone gratuitamente a disposición de los usuarios de Internet. El acceso al mismo implica su aceptación, sin reservas. La utilización de determinados servicios ofrecidos en este sitio se regirá además por las condiciones particulares previstas en cada caso, las cuales se entenderán aceptadas por el mero uso de tales servicios.`,
      },
      // #3
      {
        en: "",
        es: `3.- Se autoriza la visualización, impresión y descarga parcial del contenido de la web sólo y exclusivamente si concurren las siguientes condiciones:`,
      },
      // #3.1
      {
        en: "",
        es: `- Que sea compatible con los fines de este sitio web.`,
      },
      // #3.2
      {
        en: "",
        es: `- Que se realice con el exclusivo ánimo de obtener la información contenida para uso personal y privado. Se prohíbe expresamente su utilización con fines comerciales o para su distribución, comunicación pública, transformación o descompilación.`,
      },
      // #3.3
      {
        en: "",
        es: `- Que ninguno de los contenidos relacionados en esta web sea modificado de manera alguna.`,
      },
      // #3.4
      {
        en: "",
        es: `- Que ningún gráfico, icono o imagen disponible en esta web sea utilizado, copiado o distribuido separadamente del texto o resto de imágenes que lo acompañan.`,
      },
      // #4
      {
        en: "",
        es: `4.- EL PROPIETARIO se reserva la facultad de efectuar, en cualquier momento y sin necesidad de previo aviso, modificaciones y actualizaciones de la información contenida en su web, de la configuración y presentación de éste y de las condiciones de acceso.`,
      },
      // #5
      {
        en: "",
        es: `EL PROPIETARIO no garantiza la inexistencia de interrupciones o errores en el acceso al web, en su contenido, ni que éste se encuentre actualizado, aunque desarrollará sus mejores esfuerzos para, en su caso, evitarlos, subsanarlos o actualizarlos.`,
      },
      // #6
      {
        en: "",
        es: `Tanto el acceso a este sitio web, como el uso que pueda hacerse de la información contenida en el mismo es de la exclusiva responsabilidad de quien lo realiza. EL PROPIETARIO no responderá de ninguna consecuencia, daño o perjuicio que pudieran derivarse de dicho acceso o uso de la información. EL PROPIETARIO no se hace responsable de los posibles errores de seguridad que se puedan producir ni de los posibles daños que puedan causarse al sistema informático del usuario (hardware y software), los ficheros o documentos almacenados en el mismo, como consecuencia de la presencia de virus en el ordenador del usuario utilizado para la conexión a los servicios y contenidos de la web, de un mal funcionamiento del navegador o del uso de versiones no actualizadas del mismo.`,
      },
      // #7
      {
        en: "",
        es: `EL PROPIETARIO no asume responsabilidad alguna derivada de la concesión o contenidos de los enlaces de terceros a los que se hace referencia en la web, ni garantiza la ausencia de virus u otros elementos en los mismos que puedan producir alteraciones en el sistema informático (hardware y software), los documentos o los ficheros del usuario, excluyendo cualquier responsabilidad por los daños de cualquier clase causados al usuario por este motivo.`,
      },
      // #8
      {
        en: "",
        es: `La utilización no autorizada de la información contenida en esta web, su reventa, así como la lesión de los derechos de Propiedad Intelectual de EL PROPIETARIO, dará lugar a las responsabilidades legalmente establecidas.`,
      },
      // #9
      {
        en: "",
        es: `Todo enlace de terceros a esta web debe serlo a su página principal o de entrada.`,
      },
      // #10
      {
        en: "",
        es: `EL PROPIETARIO y el usuario, con renuncia expresa a cualquier otro fuero, se someten a los juzgados y tribunales de SEVILLA (España) para cualquier controversia que pudiera derivarse del acceso a esta web. En el caso de que el usuario tenga su domicilio fuera de España, EL PROPIETARIO y el usuario se someten, con renuncia expresa a cualquier otro fuero, a los juzgados y tribunales de SEVILLA (España).`,
      },
    ],
  },
  // Terms and Conditions
  guestLanding: {
    meta: {
      title: {
        en: "Wedding party!",
        es: "💍 ¡Te vas de boda! 🥳",
      },
      description: {
        en:
          "Application to manage weddings tasks like guests invitations, tables planning or menus.",
        es:
          "Sitio web para organizar bodas. Gestiona listas de invitados, listas de regalos y planifica las mesas y el catering.",
      },
    },
    title: {
      en: "Wedding party!",
      es: "💍 ¡Te vas de boda! 🥳",
    },
    subtitle: {
      en: "But you should confirm your assistance first",
      es: "Pero antes debes confirmar tu asistencia",
    },
    mainGuestTitle: {
      en: "Do you want to modify your personal info?",
      es: "¿Quieres modificar tus datos?",
    },
    companionsTitle: {
      en: "Do you have companion(s)?",
      es: "¿Vienes con alguien?",
    },
    assistance: {
      yes: {
        en: "Yes!",
        es: "¡Sí!",
      },
      no: {
        en: "I can not assit",
        es: "No puedo ir",
      },
      pending: {
        en: "I do not know yet",
        es: "Aún no lo sé",
      },
    },
    addCompanion: {
      en: "Add",
      es: "Añadir",
    },
    deleteCompanion: {
      en: "Delete",
      es: "Quitar",
    },
    updateInfo: {
      en: "Update info",
      es: "Actualizar información",
    },
  },
};
